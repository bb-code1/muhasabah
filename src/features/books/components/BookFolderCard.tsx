...
import { BookFolder } from '@prisma/client';

interface Props {
  folder: BookFolder;
  count: number;
  onClick: (id: number) => void;
  onRename: (folder: BookFolder, e: React.MouseEvent) => void;
  onDelete: (id: number, e: React.MouseEvent) => void;
}

export default function BookFolderCard({ folder, count, onClick, onRename, onDelete }: Props) {
  return (
    <div onClick={() => onClick(folder.id)} className="card"
      ...
      <div style={{ flex: 1, minWidth: 0 }}>
        ...
        <p style={{ margin: '2px 0 0 0', fontSize: '12px', color: 'var(--c-on-surface-variant)', fontWeight: 600 }}>{count} item{count !== 1 ? 's' : ''}</p>
      </div>
