...
import { sleepingAzkaar } from '@/features/religious/data/azkaar';
import { ArrowLeft, BookOpen } from 'lucide-react';

export default function SleepingAzkaarPage() {
  return (
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: '16px', marginBottom: '32px' }}>
        ...
        <div>
          <h1 className="text-headline-md" style={{ margin: 0, fontWeight: 800, color: 'var(--c-on-surface)' }}>
            Azkaar Before Sleeping
          </h1>
          <p className="text-body-md text-on-surface-variant" style={{ margin: 0 }}>
            Authentic supplications to recite before going to bed
          </p>
        </div>
      </div>

      <div style={{ display: 'flex', flexDirection: 'column', gap: '24px' }}>
        {sleepingAzkaar.map((azkaar, index) => (
          <div key={azkaar.id} className="card" style={{ display: 'flex', flexDirection: 'column', gap: '16px', position: 'relative', overflow: 'hidden' }}>
            ...
            {/* Title */}
            {azkaar.title && (
              <h3 className="text-title-md" style={{ margin: 0, paddingRight: '30px', fontWeight: 700, color: 'var(--c-on-surface)' }}>
                {azkaar.title}
              </h3>
            )}
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
              ...
              <div style={{ fontSize: '13px', color: 'var(--c-on-surface-variant)' }}>
                <span style={{ fontWeight: 600 }}>Reference:</span> {azkaar.reference}
              </div>
            </div>
